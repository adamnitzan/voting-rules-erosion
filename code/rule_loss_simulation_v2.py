...
def generate_one_rule_results_for_one_profile(
    profile: svvamp.Profile, rule: VotingRule
):
    """
...
def simulate_one_profile(random_sample, all_rules: List[VotingRule]):
    """
...
def simulate_voting_for_specific_nc_nv(
    ...
    if distribution == "cubic":
        random_profile = svvamp.GeneratorProfileCubicUniform(n_v=n_v, n_c=n_c)
    elif distribution == "box":
        random_profile = svvamp.GeneratorProfileEuclideanBox(
            n_v=n_v, n_c=n_c, box_dimensions=np.ones(2)
        )
    elif distribution == "IANC":
        random_profile = svvamp.GeneratorProfileIanc(n_v=n_v, n_c=n_c)
    else:
